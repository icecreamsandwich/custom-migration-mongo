  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($result) {
      case SAVED_NEW:
        ...
        $this->logger('custom_migration_mongo')->notice('Created new custom migration mongo %label', $logger_arguments);
        break;

      case SAVED_UPDATED:
        $this->messenger()->addStatus($this->t('The custom migration mongo %label has been updated.', $message_arguments));
        $this->logger('custom_migration_mongo')->notice('Updated custom migration mongo %label.', $logger_arguments);
        break;
    }

    $form_state->setRedirect('entity.custom_migration_mongo.canonical', ['custom_migration_mongo' => $entity->id()]);
